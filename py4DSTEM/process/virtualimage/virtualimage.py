...
def get_circ_mask(size_x, size_y, R=1):
    ...
    return np.fromfunction(
        lambda x, y: (
            ((x + 0.5) / (size_x / 2.0) - 1) ** 2
            + ((y + 0.5) / (size_y / 2.0) - 1) ** 2
        )
        < R ** 2,
        (size_x, size_y),
    )
...
def get_annular_mask(size_x, size_y, R):
    ...
    return np.logical_xor(
        get_circ_mask(size_x, size_y), get_circ_mask(size_x, size_y, R)
    )
...
def get_virtual_image_annular_integrate(datacube, slice_x, slice_y, R):
    ...
    """
    mask = get_annular_mask(
        slice_x.stop - slice_x.start, slice_y.stop - slice_y.start, R
    )
    try:
...
def get_virtual_image_rect_diffX(datacube, slice_x, slice_y):
    ...
    try:
        ...
        img = datacube.data[:, :, slice_left, slice_y].sum(axis=(2, 3)).astype(
            "int64"
        ) - datacube.data[:, :, slice_right, slice_y].sum(axis=(2, 3)).astype("int64")
        return img, 1
    except ValueError:
        return 0, 0
...
def get_virtual_image_rect_diffY(datacube, slice_x, slice_y):
    ...
    try:
        ...
        img = datacube.data[:, :, slice_x, slice_bottom].sum(axis=(2, 3)).astype(
            "int64"
        ) - datacube.data[:, :, slice_x, slice_top].sum(axis=(2, 3)).astype("int64")
        return img, 1
    except ValueError:
        return 0, 0
...
def get_virtual_image_circ_diffX(datacube, slice_x, slice_y):
    ...
    try:
        ...
        img = np.ndarray.astype(
            np.sum(
                datacube.data[:, :, slice_left, slice_y]
                * mask[: slice_left.stop - slice_left.start, :],
                axis=(2, 3),
            )
            - np.sum(
                datacube.data[:, :, slice_right, slice_y]
                * mask[slice_right.start - slice_right.stop :, :],
                axis=(2, 3),
            ),
            "int64",
        )
...
def get_virtual_image_circ_diffY(datacube, slice_x, slice_y):
    ...
    try:
        ...
        img = np.ndarray.astype(
            np.sum(
                datacube.data[:, :, slice_x, slice_bottom]
                * mask[:, : slice_bottom.stop - slice_bottom.start],
                axis=(2, 3),
            )
            - np.sum(
                datacube.data[:, :, slice_x, slice_top]
                * mask[:, slice_top.start - slice_top.stop :],
                axis=(2, 3),
            ),
            "int64",
        )
...
def get_virtual_image_rect_CoMX(datacube, slice_x, slice_y):
    ...
    """
    ry, rx = np.meshgrid(
        np.arange(slice_y.stop - slice_y.start), np.arange(slice_x.stop - slice_x.start)
    )
    try:
        img = np.sum(
            datacube.data[:, :, slice_x, slice_y] * rx, axis=(2, 3)
        ) / datacube.data[:, :, slice_x, slice_y].sum(axis=(2, 3))
        return img, 1
    except ValueError:
        return 0, 0
...
def get_virtual_image_rect_CoMY(datacube, slice_x, slice_y):
    ...
    try:
        img = np.sum(
            datacube.data[:, :, slice_x, slice_y] * ry, axis=(2, 3)
        ) / datacube.data[:, :, slice_x, slice_y].sum(axis=(2, 3))
        return img, 1
    except ValueError:
        return 0, 0
...
def get_virtual_image_circ_CoMX(datacube, slice_x, slice_y):
    ...
    try:
        img = np.sum(
            datacube.data[:, :, slice_x, slice_y] * rx * mask, axis=(2, 3)
        ) / np.sum(datacube.data[:, :, slice_x, slice_y] * mask, axis=(2, 3))
        return img, 1
    except ValueError:
        return 0, 0
...
def get_virtual_image_circ_CoMY(datacube, slice_x, slice_y):
    ...
    try:
        img = np.sum(
            datacube.data[:, :, slice_x, slice_y] * ry * mask, axis=(2, 3)
        ) / np.sum(datacube.data[:, :, slice_x, slice_y] * mask, axis=(2, 3))
        return img, 1
    except ValueError:
        return 0, 0
