...
def fit_lattice_vectors(
    bragg_peaks, bragg_directions, x0, y0, maxPeakSpacing=20, minNumPeaks=5
):
    """
    ...
    assert np.all(
        [name in bragg_peaks.dtype.names for name in ("qx", "qy", "intensity")]
    )
    assert isinstance(bragg_directions, PointList)
    assert np.all(
        [name in bragg_directions.dtype.names for name in ("qx", "qy", "h", "k")]
    )
    ...
    for i in range(bragg_peaks.length):
        r2 = (bragg_peaks.data["qx"][i] - bragg_directions.data["qx"]) ** 2 + (
            bragg_peaks.data["qy"][i] - bragg_directions.data["qy"]
        ) ** 2
        ind = np.argmin(r2)
...
def fit_lattice_vectors_all_DPs(
    ...
    uv_map = RealSlice(
        data=np.zeros((bragg_peaks.shape[0], bragg_peaks.shape[1], 6)),
    ...
    for (Rx, Ry) in tqdmnd(bragg_peaks.shape[0], bragg_peaks.shape[1]):
        ...
        ux, uy, vx, vy, error = fit_lattice_vectors(
            bragg_peaks_curr, bragg_directions, x0, y0, maxPeakSpacing, minNumPeaks
        )
        # Store data
        if ux is not None:
...
def fit_lattice_vectors_masked(
    bragg_peaks, bragg_directions, x0, y0, mask, maxPeakSpacing=20, minNumPeaks=5
):
    """
