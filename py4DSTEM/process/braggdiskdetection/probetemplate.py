...
def get_average_probe_from_vacuum_scan(
    datacube, mask_threshold=0.2, mask_expansion=12, mask_opening=3, verbose=False
):
    """
    ...
    for n in tqdmnd(range(1, datacube.R_N)):
        ...
        if verbose:
            print(
                "Shifting and averaging diffraction pattern {} of {}.".format(
                    n, datacube.R_N
                )
            )
...
def get_average_probe_from_vacuum_stack(
    data, mask_threshold=0.2, mask_expansion=12, mask_opening=3
):
    """
...
def get_average_probe_from_ROI(
    datacube,
    ROI,
    mask_threshold=0.2,
    mask_expansion=12,
...
def get_probe_kernel_logistictrench(probe, radius, trenchwidth, blurwidth):
    ...
    # Calculate logistic function
    logistic_annulus = 1 / (1 + np.exp(4 * qr / blurwidth)) - 1 / (
        1 + np.exp(4 * (qr - trenchwidth) / blurwidth)
    )
