...
def find_Bragg_disks_selected(
    ...
    print(
        "Analyzed {} diffraction patterns in {}h {}m {}s".format(
            len(Rx), int(t / 3600), int((t % 3600) / 60), int(t % 60)
        )
    )
    ...
    if show:
        plot_bragg_disks(
            peaks, datacube, Rx, Ry, show_image, show_scale, show_power, filter_function
        )

    return tuple(peaks)


def find_Bragg_disks_serial(
    datacube,
    probe,
    corrPower=1,
    sigma=2,
