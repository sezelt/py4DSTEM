...
def plot_bragg_disks(
    peaks, datacube, Rx, Ry, image=None, scale=200, power=0.25, filter_function=None
):
    from cycler import cycler

    gridsize = int(np.ceil(np.sqrt(len(Rx) + 1)))
    fig, ax = plt.subplots(
        int(np.ceil((len(Rx) + 1) / gridsize)), gridsize, figsize=(12, 12)
    )

    ax[0, 0].matshow(
        image if image is not None else np.zeros((datacube.R_Nx, datacube.R_Ny))
    )
