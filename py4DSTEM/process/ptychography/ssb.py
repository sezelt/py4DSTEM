...
def aperture_xp(qx, qy, lam, alpha_max, edge=2):
    ...
    if edge > 0:
        ...
        arr[ind] = 0.5 * (
            1 - xp.sin(np.pi / (2 * dEdge) * (ktheta[ind] / alpha_max - 1))
        )
    return arr
...
def weak_phase_reconstruction(dc: DataCube, verbose=False, use_cuda=True):
    ...
    assert (
        "accelerating_voltage" in dc.metadata.microscope
    ), "metadata.microscope dictionary missing key: accelerating_voltage"
    assert (
        "convergence_semiangle_mrad" in dc.metadata.microscope
    ), "metadata.microscope dictionary missing key: convergence_semiangle_mrad"

    assert (
        "K_pix_size" in dc.metadata.calibration
    ), "metadata.calibration dictionary missing key: K_pix_size"
    assert (
        "R_pix_size" in dc.metadata.calibration
    ), "metadata.calibration dictionary missing key: R_pix_size"
    assert (
        "R_to_K_rotation_degrees" in dc.metadata.calibration
    ), "metadata.calibration dictionary missing key: R_to_K_rotation_degrees"
    ...
    if cuda_is_available:
        ...
        single_sideband_kernel[blockspergrid, threadsperblock](
            G,
            strides,
            Qx,
            Qy,
        ...
        Kplus = np.sqrt(
            (Kx + Qx[:, :, None, None]) ** 2 + (Ky + Qy[:, :, None, None]) ** 2
        )
        Kminus = np.sqrt(
            (Kx - Qx[:, :, None, None]) ** 2 + (Ky - Qy[:, :, None, None]) ** 2
        )
        ...
        for ix, qx in enumerate(Qx[0]):
            ...
            for iy, qy in enumerate(Qy[:, 0]):
                x = Kx + qx
                y = Ky + qy
                A_KplusQ[iy, ix] = np.exp(
                    1j * cartesian_aberrations(x, y, lam, C)
                ) * aperture_xp(x, y, lam, alpha_rad, edge=0)
                # A_KplusQ *= 1e4

                x = Kx - qx
                y = Ky - qy
                A_KminusQ[iy, ix] = np.exp(
                    1j * cartesian_aberrations(x, y, lam, C)
                ) * aperture_xp(x, y, lam, alpha_rad, edge=0)
                # A_KminusQ *= 1e4
