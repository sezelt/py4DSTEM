...
class DataViewer(QtWidgets.QMainWindow):
    ...
    def setup_control_widget(self):
        ...
        self.settings.diffraction_scaling_mode.connect_bidir_to_widget(
            self.control_widget.buttonGroup_DiffractionMode
        )
        self.settings.diffraction_scaling_mode.updated_value.connect(
            self.diffraction_scaling_changed
        )
        ...
        self.settings.arrowkey_mode.connect_bidir_to_widget(
            self.control_widget.virtualDetectors.widget.buttonGroup_ArrowkeyMode
        )
    ...
    def Unidentified_file(self, fname):
        msg = QtWidgets.QMessageBox()
        msg.setText(
            "Couldn't open {0} as it doesn't conform to currently implemented py4DSTEM standards".format(
                fname
            )
        )
        msg.setStandardButtons(QtWidgets.QMessageBox.Ok)
    ...
    def load_file(self):
        ...
        if (
            self.control_widget.widget_LoadPreprocessSave.widget.loadRadioAuto.isChecked()
        ):
            # auto mode
            ...
            self.control_widget.widget_LoadPreprocessSave.widget.loadRadioMMAP.isChecked()
        ):
    ...
    def crop_data(self):
        ...
        if self.control_widget.checkBox_Crop_Diffraction.isChecked():
            ...
            crop_Qx_min, crop_Qx_max = (
                max(0, crop_Qx_min),
                min(self.datacube.Q_Nx, crop_Qx_max),
            )
            crop_Qy_min, crop_Qy_max = (
                max(0, crop_Qy_min),
                min(self.datacube.Q_Ny, crop_Qy_max),
            )
            # Move ROI selector
            ...
            # Crop data
            self.datacube.crop_data_diffraction(
        ...
        if self.control_widget.checkBox_Crop_Real.isChecked():
            ...
            slices_r, transforms_r = self.crop_roi_real.getArraySlice(
                self.datacube.data[:, :, 0, 0], self.real_space_widget.getImageItem()
            )
            ...
            crop_Rx_min, crop_Rx_max = (
                max(0, crop_Rx_min),
                min(self.datacube.R_Nx, crop_Rx_max),
            )
            crop_Ry_min, crop_Ry_max = (
                max(0, crop_Ry_min),
                min(self.datacube.R_Ny, crop_Ry_max),
            )
            # Move point selector
            x0, y0 = (
                self.real_space_point_selector.x(),
    ...
    def save_editMetadata(self):
        ...
        for i in range(self.EditMetadataWidget.tabs.count()):
            ...
            metadata_dict_name = [
                name
                for name in self.datacube.metadata.__dict__.keys()
        ...
        if detector_mode == 0:
            if detector_shape == 0:
                ...
                self.get_virtual_image = (
                    self.datacube.get_virtual_image_annular_integrate
                )
            else:
                raise ValueError(
        ...
        if success:
            ...
            if self.settings.diffraction_scaling_mode.val == 1:
                ...
                self.diffraction_space_view = np.log(
                    self.diffraction_space_view
                    - np.min(self.diffraction_space_view)
                    + 1
                )
            elif self.settings.diffraction_scaling_mode.val == 3:
                # EWPC mode
                h = (
                    np.hanning(self.datacube.Q_Nx)[:, np.newaxis]
                    @ np.hanning(self.datacube.Q_Ny)[np.newaxis, :]
                )
                self.diffraction_space_view = (
                    np.abs(
                        np.fft.fftshift(
            ...
            self.diffraction_space_widget.setImage(
                self.diffraction_space_view, autoLevels=False, autoRange=False
            )
        else:
            pass
        return
    ...
    def update_real_space_view(self):
        ...
        # Annular detector
            ...
            R = 0.5 * (
                (slice_inner_x.stop - slice_inner_x.start)
            ...
            print(
                "Error: unknown detector shape value {}.  Must be 0, 1, or 2.".format(
                    detector_shape
                )
            )
    ...
    def keyPressEvent(self, e):
        ...
        if mode == 0:  # we are in realspace mode
            ...
            if e.key() == QtCore.Qt.Key_Left:
                ...
                self.settings.arrowkey_mode.update_value(
                    2
                )  # relase keyboard control if you press anything else
