...
from ...process.braggdiskdetection import (
    get_average_probe_from_ROI,
    get_probe_kernel,
    get_probe_kernel_subtrgaussian,
)
...
from ...process.latticevectors import (
    get_radon_scores,
    get_lattice_directions_from_scores,
    get_lattice_vector_lengths,
    generate_lattice,
)
...
class ProbeKernelTab(QtWidgets.QWidget):
    def __init__(self, main_window=None):
        ...
        leftpolicy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
        leftpolicy.setHorizontalStretch(1)
        rightpolicy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
    ...
    def update_RS(self):
        try:
            ...
            slices, transforms = self.diffraction_ROI.getArraySlice(
                dc.data[0, 0, :, :], self.diffraction_widget.getImageItem()
            )
            slice_x, slice_y = slices

            new_real_space_view, success = dc.get_virtual_image_rect_integrate(
                slice_x, slice_y
            )
            if success:
                self.realspace_widget.setImage(
                    new_real_space_view ** 0.5, autoLevels=True
                )
            else:
                pass
        except:
    ...
    def update_DP(self):
        try:
            dc = self.main_window.strain_window.vac_datacube
            slices, transforms = self.realspace_ROI.getArraySlice(
                dc.data[:, :, 0, 0], self.realspace_widget.getImageItem()
            )
            slice_x, slice_y = slices
...
class VacuumDCTab(QtWidgets.QWidget):
    ...
    def load_file(self):
        ...
        fpath = QtWidgets.QFileDialog.getOpenFileName(
            self, "Choose Vacuum DataCube", currentpath
        )
        ...
        if fpath[0]:
            ...
            if self.loadRadioAuto.isChecked():
                ...
                if binning > 1:
                    self.main_window.strain_window.vac_datacube.bin_data_diffraction(
                        binning
                    )
                ...
                self.main_window.strain_window.vac_datacube = read(
                    fname, load="gatan_bin"
                )
                if binning > 1:
                    self.main_window.strain_window.vac_datacube.bin_data_mmap(binning)
...
class UseMainDCTab(QtWidgets.QWidget):
    def __init__(self, main_window):
        ...
        self.button_copy_DC.clicked.connect(
            main_window.strain_window.copy_vac_DC_from_browser
        )
...
class ProkeKernelSettings(QtWidgets.QGroupBox):
    ...
    def generate_probe(self):
        ...
        (
            slices,
            transforms,
        ) = self.main_window.strain_window.probe_kernel_tab.realspace_ROI.getArraySlice(
            dc.data[:, :, 0, 0],
        ...
        (
            slices,
            transforms,
        ) = self.main_window.strain_window.probe_kernel_tab.diffraction_ROI.getArraySlice(
            dc.data[0, 0, :, :],
        ...
        self.probe = get_average_probe_from_ROI(
            dc,
            RS_mask,
        ...
        if use_gaussian:
            self.probe_kernel = get_probe_kernel_subtrgaussian(
                self.probe, sigma_probe_scale=gaussian_scale
            )
        else:
        ...
        linetrace_left = np.sum(
            self.probe_kernel[-linetracelength:, :linetracewidth], axis=(1)
        )
        linetrace_right = np.sum(
            self.probe_kernel[:linetracelength, :linetracewidth], axis=(1)
        )
        linetrace = np.concatenate([linetrace_left, linetrace_right])
        pkdisplay.probe_kernel_linetrace_plot.setData(
            np.arange(len(linetrace)), linetrace
        )
    ...
    def accept_probe(self):
        ...
        self.main_window.strain_window.tab_widget.setTabEnabled(
            self.main_window.strain_window.bragg_disk_tab_index, True
        )
        self.main_window.strain_window.probe_kernel = self.probe_kernel
        self.main_window.strain_window.ProbeKernelDS = DiffractionSlice(
            data=self.probe_kernel
        )
...
class BraggDiskTab(QtWidgets.QWidget):
    def __init__(self, main_window=None):
        ...
        self.bragg_disk_preview_pane = BraggDiskPreviewPane(
        ...
        # connect the ROIs
        self.bragg_disk_preview_pane.bragg_preview_realspace_1_selector.sigRegionChangeFinished.connect(
            self.update_views
        )
        self.bragg_disk_preview_pane.bragg_preview_realspace_2_selector.sigRegionChangeFinished.connect(
            self.update_views
        )
        self.bragg_disk_preview_pane.bragg_preview_realspace_3_selector.sigRegionChangeFinished.connect(
            self.update_views
        )

        # connect the settings buttons to also update the DPs:
        self.bragg_disk_settings_pane.corr_power_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.sigma_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.edge_boundary_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.min_relative_intensity_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.relative_to_peak_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.min_peak_spacing_spinBox.valueChanged.connect(
            self.update_views
        )
        self.bragg_disk_settings_pane.max_num_peaks_spinBox.valueChanged.connect(
            self.update_views
        )

        # connect the peakfinding button
        self.bragg_disk_control_box.start_peakfinding_button.clicked.connect(
            self.find_all_bragg_disks
        )
        self.main_window.strain_window.bragg_peak_progressbar = (
            self.bragg_disk_control_box.bragg_peak_progressbar
        )

        # set sizing
        left_size_policy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
        right_size_policy = QtWidgets.QSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
    ...
    def find_all_bragg_disks(self):
        ...
        self.bragg_disk_control_box.bragg_peak_progressbar.setMaximum(
            self.main_window.datacube.R_N
        )

        try:
            self.main_window.strain_window.braggdisks = find_Bragg_disks(
                self.main_window.datacube,
            ...
            self.main_window.strain_window.braggdisks_corrected = (
                self.main_window.strain_window.braggdisks.copy()
            )
            # now enable the next tab!
            self.main_window.strain_window.bragg_peaks_accepted = True
            self.main_window.strain_window.tab_widget.setTabEnabled(
                self.main_window.strain_window.lattice_vector_tab_index, True
            )
    ...
    def update_views(self):
        if self.main_window.strain_window.probe_kernel_accepted:
            ...
            try:
                ...
                newscatter1 = pg.ScatterPlotItem(
                    size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 120)
                )
                newscatter2 = pg.ScatterPlotItem(
                    size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 120)
                )
                newscatter3 = pg.ScatterPlotItem(
                    size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 0, 0, 120)
                )
            ...
            (
                new_diffraction_space_view,
                success,
            ) = self.main_window.datacube.get_diffraction_space_view(xc, yc)
            if success:
                braggviews.bragg_preview_DP_1.setImage(
                    new_diffraction_space_view, autoLevels=True, autoRange=False
                )
            ...
            if success:
                braggviews.bragg_preview_DP_2.setImage(
                    new_diffraction_space_view, autoLevels=True, autoRange=False
                )
            ...
            if success:
                braggviews.bragg_preview_DP_3.setImage(
                    new_diffraction_space_view, autoLevels=True, autoRange=False
                )
    ...
    def find_selected_bragg_disks(self):
        ...
        peaks = find_Bragg_disks_selected(
            self.main_window.datacube,
            self.main_window.strain_window.probe_kernel,
            xs,
            ys,
        ...
        try:
            spots1 = [
                {"pos": [peaks[0].data["qx"][i], peaks[0].data["qy"][i]], "data": 1}
                for i in range(peaks[0].length)
            ]
            newscatter1 = pg.ScatterPlotItem(
        ...
        try:
            spots2 = [
                {"pos": [peaks[1].data["qx"][i], peaks[1].data["qy"][i]], "data": 1}
                for i in range(peaks[1].length)
            ]
            newscatter2 = pg.ScatterPlotItem(
                size=10, pen=pg.mkPen(None), brush=pg.mkBrush(0, 255, 0, 120)
            )
        ...
        try:
            spots3 = [
                {"pos": [peaks[2].data["qx"][i], peaks[2].data["qy"][i]], "data": 1}
                for i in range(peaks[2].length)
            ]
            newscatter3 = pg.ScatterPlotItem(
                size=10, pen=pg.mkPen(None), brush=pg.mkBrush(0, 255, 0, 120)
            )
...
class BraggDiskPreviewPane(QtWidgets.QGroupBox):
    def __init__(self, main_window=None):
        ...
        self.bragg_preview_realspace_1_selector = pg_point_roi(
            self.bragg_preview_realspace_1.getView()
        )
        ...
        self.bragg_preview_realspace_2_selector = pg_point_roi(
            self.bragg_preview_realspace_2.getView()
        )
        ...
        self.bragg_preview_realspace_3_selector = pg_point_roi(
            self.bragg_preview_realspace_3.getView()
        )
...
class LatticeVectorTab(QtWidgets.QWidget):
    def __init__(self, main_window=None):
        ...
        self.lattice_scatter = pg.ScatterPlotItem(
            size=10, pen=pg.mkPen(None), brush=pg.mkBrush(255, 255, 255, 120)
        )
        ...
        self.viz_pane.lattice_vector_ROI.sigRegionChanged.connect(
            self.lattice_vectors_moved
        )

        # setup connections
        self.settings_pane.shifts_use_fits_checkbox.stateChanged.connect(
            self.update_BVM
        )

        self.settings_pane.direction_sigma_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.directions_min_spacing_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.directions_min_rel_int_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.directions_index1_spinbox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.directions_index2_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.lengths_spacing_thresh_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.lengths_sigma_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.lengths_min_spacing_spinBox.valueChanged.connect(
            self.update_lattice
        )
        self.settings_pane.lengths_min_rel_int_spinBox.valueChanged.connect(
            self.update_lattice
        )
    ...
    def accept_lattice(self):
        ...
        self.main_window.strain_window.BVMDS = DiffractionSlice(
            data=self.main_window.strain_window.BVM
        )
    ...
    def update_lattice(self):
        if self.main_window.strain_window.sinogram_accepted & (
            not self.settings_pane.freeform_enable.isChecked()
        ):
            try:
                ...
                minRelativeIntensity = (
                    self.settings_pane.directions_min_rel_int_spinBox.value()
                )
                ...
                self.x0, self.y0 = np.unravel_index(
                    np.argmax(
                        gaussian_filter(
                            self.main_window.strain_window.BVM * mask, sigma=2
                        )
                    ),
                    (dc.Q_Nx, dc.Q_Ny),
                )
                ...
                self.ideal_lattice = generate_lattice(
                    self.ux,
                    self.uy,
                    self.vx,
                    self.vy,
                ...
                spots = [
                    {
                        "pos": [
                            self.ideal_lattice.data["qx"][i],
                            self.ideal_lattice.data["qy"][i],
                        ],
                        "data": 1,
                    }
                    for i in range(self.ideal_lattice.length)
                ]
                newscatter = pg.ScatterPlotItem(
                    size=10, pen=pg.mkPen(None), brush=pg.mkBrush(0, 255, 0, 120)
                )
                ...
                self.viz_pane.lattice_vector_ROI.setPoints(
                    (
                        [self.x0 + self.ux, self.y0 + self.uy],
                        [self.x0, self.y0],
                        [self.x0 + self.vx, self.y0 + self.vy],
                    )
                )

            except Exception as exc:
                print(
                    "Failed to find sinogram, pr update lattice vectors... Check the direct beam ROI."
                )
                print(format(exc))
    ...
    def lattice_vectors_moved(self):
        ...
        if self.settings_pane.freeform_enable.isChecked():
            try:
                ...
                self.ux, self.uy = (
                    float(pts[0][0]) - float(self.x0),
                    float(pts[0][1]) - float(self.y0),
                )
                self.vx, self.vy = (
                    float(pts[2][0]) - float(self.x0),
                    float(pts[2][1]) - float(self.y0),
                )
    ...
    def update_BVM(self):
        ...
        if self.main_window.strain_window.bragg_peaks_accepted:
            xshifts, yshifts, bvm_center = get_diffraction_shifts(
                self.main_window.strain_window.braggdisks, dc.Q_Nx, dc.Q_Ny
            )
            ...
            self.main_window.strain_window.braggdisks_corrected = shift_braggpeaks(
                self.main_window.strain_window.braggdisks, xshifts_fit, yshifts_fit
            )
            self.main_window.strain_window.braggdisks_corrected.name = (
                "braggpeaks_shiftcorrected"
            )

            BVM = get_bragg_vector_map(
                self.main_window.strain_window.braggdisks_corrected, dc.Q_Nx, dc.Q_Ny
            )
...
class LatticeVectorSettingsPane(QtWidgets.QGroupBox):
    def __init__(self, main_window=None):
        ...
        directionform.addRow(
            "Minimum Relative Intensity", self.directions_min_rel_int_spinBox
        )
        ...
        lengthform.addRow(
            "Minimum Relative Intensity", self.lengths_min_rel_int_spinBox
        )
...
class StrainMapTab(QtWidgets.QWidget):
    ...
    def update_strain(self):
        if self.main_window.strain_window.lattice_vectors_accepted:
            ...
            slices, transforms = self.RS_refROI.getArraySlice(
                mask.astype(float), self.RS_view.getImageItem()
            )
            ...
            n_stds = 3
            e_xx_ave, e_xx_std = (
                np.average(sm.slices["e_xx"][mask]),
                np.std(sm.slices["e_xx"][mask]),
            )
            e_yy_ave, e_yy_std = (
                np.average(sm.slices["e_yy"][mask]),
                np.std(sm.slices["e_yy"][mask]),
            )
            e_xy_ave, e_xy_std = (
                np.average(sm.slices["e_xy"][mask]),
                np.std(sm.slices["e_xy"][mask]),
            )
            theta_ave, theta_std = (
                np.average(sm.slices["theta"][mask]),
                np.std(sm.slices["theta"][mask]),
            )
            ...
            self.theta_view.getHistogramWidget().setLevels(
    ...
    def update_RS(self):
        dc = self.main_window.datacube
        slices, transforms = self.DP_ROI.getArraySlice(
            dc.data[0, 0, :, :], self.DP_view.getImageItem()
        )
        slice_x, slice_y = slices
    ...
    def export_processing(self):
        ...
        f = QtWidgets.QFileDialog.getSaveFileName(
            self,
            "Export Processing (do not save DataCube)...",
            currentpath,
            "py4DSTEM Files (*.h5)",
        )
    ...
    def export_all(self):
        ...
        currentpath = os.path.splitext(self.main_window.settings.data_filename.val)[0]
        f = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save file...", currentpath, "py4DSTEM Files (*.h5)"
        )
