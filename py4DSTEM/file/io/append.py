...
def append_from_dataobject_list(dataobject_list, filepath):
    ...
    assert is_py4DSTEM_file(
        filepath
    ), "filepath parameter must point to an existing py4DSTEM file."
    browser = FileBrowser(filepath)
    if browser.version[0] == 0:
        assert (
            browser.version[1] >= 3
        ), "appending to py4DSTEM files only supported in v0.3 and higher."
    ...
    try:
        ...
        print(
            "This can occur if your datacube is memory-mapped from a py4DSTEM h5 file."
        )
        print(
            f"To force close the file, closing any dataobjects open from it, run: py4DSTEM.file.io.close_h5_at_path('{filepath}')"
        )
