...
class FileBrowser(object):
    ...
    def set_object_lookup_info(self):
        if self.version == (0, 7):
            ...
            print(
                "Error: unknown py4DSTEM version {}.{}.".format(
    ...
    def set_object_lookup_info_v0_7(self):
        ...
        self.N_diffractionslices = len(
            self.file[self.topgroup + "data/diffractionslices"]
        )
        ...
        self.N_dataobjects = np.sum(
            [
                self.N_datacubes,
                self.N_counted,
                self.N_diffractionslices,
                self.N_realslices,
                self.N_pointlists,
                self.N_pointlistarrays,
            ]
        )
        ...
        self.dataobject_lookup_arr += [
            "DiffractionSlice" for i in range(self.N_diffractionslices)
        ]
        ...
        self.dataobject_lookup_arr += [
            "PointListArray" for i in range(self.N_pointlistarrays)
        ]
    ...
    def set_object_lookup_info_v0_4(self):
        ...
        self.N_diffractionslices = len(
            self.file["4DSTEM_experiment/data/diffractionslices"]
        )
        ...
        self.N_pointlistarrays = len(
            self.file["4DSTEM_experiment/data/pointlistarrays"]
        )
        self.N_dataobjects = np.sum(
            [
                self.N_datacubes,
    ...
    def set_object_lookup_info_v0_2(self):
        ...
        self.N_diffractionslices = len(
            self.file["4DSTEM_experiment/processing/diffractionslices"]
        )
        ...
        self.N_pointlistarrays = len(
            self.file["4DSTEM_experiment/processing/pointlistarrays"]
        )
        self.N_dataobjects = np.sum(
            [
                self.N_rawdatacubes,
        ...
        self.dataobject_lookup_arr = []
        self.dataobject_lookup_arr += [
            "RawDataCube" for i in range(self.N_rawdatacubes)
        ]
        self.dataobject_lookup_arr += ["DataCube" for i in range(self.N_datacubes)]
        self.dataobject_lookup_arr += [
            "DiffractionSlice" for i in range(self.N_diffractionslices)
    ...
    def show_diffractionslices(self):
        if self.N_diffractionslices == 0:
            ...
            for index in (self.dataobject_lookup_arr == "DiffractionSlice").nonzero()[
                0
            ]:
                info = self.get_dataobject_info(index)
                name = info["name"]
    ...
    def show_pointlists(self):
        if self.N_pointlists == 0:
            ...
            for index in (self.dataobject_lookup_arr == "PointList").nonzero()[0]:
                ...
                print(
                    "{:^8}{:<36}{:^8}{:^24}".format(
                        index, name, length, str(coordinates)
                    )
                )
    ...
    def show_pointlistarrays(self):
        if self.N_pointlistarrays == 0:
            ...
            for index in (self.dataobject_lookup_arr == "PointListArray").nonzero()[0]:
                ...
                print(
                    "{:^8}{:<36}{:^8}{:^24}".format(
                        index, name, shape, str(coordinates)
                    )
                )
    ...
    def get_dataobject_info_v0_7(self, index):
        ...
        if objecttype == "DataCube":
            ...
            metadata = self.file[self.topgroup + "data/datacubes"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file[self.topgroup + "data/counted_datacubes"].keys())[
                objectindex
            ]
            R_Nx, R_Ny = self.file[self.topgroup + "data/counted_datacubes"][name][
                "data"
            ].shape
            Q_Nx = self.file[self.topgroup + "data/counted_datacubes"][name][
                "dim3"
            ].shape[0]
            Q_Ny = self.file[self.topgroup + "data/counted_datacubes"][name][
                "dim4"
            ].shape[0]
            shape = (R_Nx, R_Ny, Q_Nx, Q_Ny)
            metadata = self.file[self.topgroup + "data/counted_datacubes"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file[self.topgroup + "data/diffractionslices"].keys())[
                objectindex
            ]
            shape = self.file[self.topgroup + "data/diffractionslices"][name][
                "data"
            ].shape
            metadata = self.file[self.topgroup + "data/diffractionslices"][name].attrs[
                "metadata"
            ]
            depth = 1
            objectinfo = {
            ...
            if len(shape) == 3:
                ...
                if (
                    "S" in dim3.dtype.str
                ):  # Checks if dim3 is composed of fixed width C strings
            ...
            name = list(self.file[self.topgroup + "data/realslices"].keys())[
                objectindex
            ]
            shape = self.file[self.topgroup + "data/realslices"][name]["data"].shape
            metadata = self.file[self.topgroup + "data/realslices"][name].attrs[
                "metadata"
            ]
            depth = 1
            objectinfo = {
            ...
            name = list(self.file[self.topgroup + "data/pointlists"].keys())[
                objectindex
            ]
            coordinates = list(
                self.file[self.topgroup + "data/pointlists"][name].keys()
            )
            length = self.file[self.topgroup + "data/pointlists"][name][coordinates[0]][
                "data"
            ].shape[0]
            metadata = self.file[self.topgroup + "data/pointlists"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file[self.topgroup + "data/pointlistarrays"].keys())[
                objectindex
            ]
            coordinates = self.file[self.topgroup + "data/pointlistarrays"][name][
                "data"
            ][0, 0].dtype
            shape = self.file[self.topgroup + "data/pointlistarrays"][name][
                "data"
            ].shape
            metadata = self.file[self.topgroup + "data/pointlistarrays"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
    ...
    def get_dataobject_info_v0_6(self, index):
        ...
        if objecttype == "DataCube":
            ...
            coordinates = list(
                self.file[self.topgroup + "data/pointlistarrays"][name]["0_0"].keys()
            )
            i, j = 0, 0
            for key in list(
                self.file[self.topgroup + "data/pointlistarrays"][name].keys()
            ):
    ...
    def get_dataobject_info_v0_5(self, index):
        ...
        if objecttype == "DataCube":
            ...
            slices = list(
                self.file[self.topgroup + "data/diffractionslices"][name].keys()
            )
            slices.remove("dim1")
            slices.remove("dim2")
            if "dim3" in slices:
    ...
    def get_dataobject_info_v0_4(self, index):
        ...
        if objecttype == "DataCube":
            name = list(self.file["4DSTEM_experiment/data/datacubes"].keys())[
                objectindex
            ]
            shape = self.file["4DSTEM_experiment/data/datacubes"][name][
                "datacube"
            ].shape
            metadata = self.file["4DSTEM_experiment/data/datacubes"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/data/diffractionslices"].keys())[
                objectindex
            ]
            depth = self.file["4DSTEM_experiment/data/diffractionslices"][name].attrs[
                "depth"
            ]
            slices = list(
                self.file["4DSTEM_experiment/data/diffractionslices"][name].keys()
            )
            slices.remove("dim1")
            slices.remove("dim2")
            shape = self.file["4DSTEM_experiment/data/diffractionslices"][name][
                slices[0]
            ].shape
            metadata = self.file["4DSTEM_experiment/data/diffractionslices"][
                name
            ].attrs["metadata"]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/data/realslices"].keys())[
                objectindex
            ]
            ...
            slices.remove("dim1")
            slices.remove("dim2")
            shape = self.file["4DSTEM_experiment/data/realslices"][name][
                slices[0]
            ].shape
            metadata = self.file["4DSTEM_experiment/data/realslices"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/data/pointlists"].keys())[
                objectindex
            ]
            coordinates = list(
                self.file["4DSTEM_experiment/data/pointlists"][name].keys()
            )
            length = self.file["4DSTEM_experiment/data/pointlists"][name][
                coordinates[0]
            ]["data"].shape[0]
            metadata = self.file["4DSTEM_experiment/data/pointlists"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/data/pointlistarrays"].keys())[
                objectindex
            ]
            coordinates = list(
                self.file["4DSTEM_experiment/data/pointlistarrays"][name]["0_0"].keys()
            )
            i, j = 0, 0
            for key in list(
                self.file["4DSTEM_experiment/data/pointlistarrays"][name].keys()
            ):
                i0, j0 = int(key.split("_")[0]), int(key.split("_")[1])
                i, j = max(i0, i), max(j0, j)
            shape = (i + 1, j + 1)
            metadata = self.file["4DSTEM_experiment/data/pointlistarrays"][name].attrs[
                "metadata"
            ]
            objectinfo = {
                "name": name,
    ...
    def get_dataobject_info_v0_2(self, index):
        ...
        if objecttype == "RawDataCube":
            ...
            name = list(self.file["4DSTEM_experiment/processing/datacubes"].keys())[
                objectindex
            ]
            shape = self.file["4DSTEM_experiment/processing/datacubes"][name][
                "datacube"
            ].shape
            objectinfo = {
                "name": name,
            ...
            name = list(
                self.file["4DSTEM_experiment/processing/diffractionslices"].keys()
            )[objectindex]
            depth = self.file["4DSTEM_experiment/processing/diffractionslices"][
                name
            ].attrs["depth"]
            slices = list(
                self.file["4DSTEM_experiment/processing/diffractionslices"][name].keys()
            )
            slices.remove("dim1")
            slices.remove("dim2")
            shape = self.file["4DSTEM_experiment/processing/diffractionslices"][name][
                slices[0]
            ].shape
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/processing/realslices"].keys())[
                objectindex
            ]
            depth = self.file["4DSTEM_experiment/processing/realslices"][name].attrs[
                "depth"
            ]
            slices = list(
                self.file["4DSTEM_experiment/processing/realslices"][name].keys()
            )
            slices.remove("dim1")
            slices.remove("dim2")
            shape = self.file["4DSTEM_experiment/processing/realslices"][name][
                slices[0]
            ].shape
            objectinfo = {
                "name": name,
            ...
            name = list(self.file["4DSTEM_experiment/processing/pointlists"].keys())[
                objectindex
            ]
            coordinates = list(
                self.file["4DSTEM_experiment/processing/pointlists"][name].keys()
            )
            length = self.file["4DSTEM_experiment/processing/pointlists"][name][
                coordinates[0]
            ]["data"].shape[0]
            objectinfo = {
                "name": name,
            ...
            name = list(
                self.file["4DSTEM_experiment/processing/pointlistarrays"].keys()
            )[objectindex]
            coordinates = list(
                self.file["4DSTEM_experiment/processing/pointlistarrays"][name][
                    "0_0"
                ].keys()
            )
            i, j = 0, 0
            for key in list(
                self.file["4DSTEM_experiment/processing/pointlistarrays"][name].keys()
            ):
                i0, j0 = int(key.split("_")[0]), int(key.split("_")[1])
                i, j = max(i0, i), max(j0, j)
            shape = (i + 1, j + 1)
    ...
    def get_dataobject(self, dataobject, **kwargs):
        ...
        """
        assert isinstance(
            dataobject, (int, np.integer, str)
        ), "dataobject must be int or str"
        ...
        if self.version == (0, 6):
            ...
            raise Exception(
                "Error: unrecognized py4DSTEM version {}.{}.".format(
                    self.version[0], self.version[1]
                )
            )
    ...
    def get_dataobject_index_from_name(self, name):
        ...
        if len(objects) > 1:
            raise Exception(
                "Multiple dataobjects found with name {}, at indices {}.".format(
                    name, objects
                )
            )
        elif len(objects) == 1:
    ...
    def get_dataobject_v0_7(self, index, **kwargs):
        ...
        if metadata_index == -1:
            ...
            if self.metadataobjects_in_memory[metadata_index] is None:
                self.metadataobjects_in_memory[
                    metadata_index
                ] = self.get_metadataobject(metadata_index)
        ...
        if objecttype == "DataCube":
            ...
            if mmap:
                ...
                data = np.array(
                    self.file[self.topgroup + "data/datacubes"][name]["data"]
                )
            ...
            if coordinates.fields is None:
                ...
                index_coords = self.file[self.topgroup + "data/counted_datacubes"][
                    name
                ]["index_coords"]
            ...
            if mmap:
                ...
                pla = PointListArray(
                    coordinates=coordinates, shape=shape[:2], name=name
                )

                for (i, j) in tqdmnd(
                    shape[0], shape[1], desc="Reading electrons", unit="DP"
                ):
            ...
            Q_Ny = shape[1]
            data = np.array(
                self.file[self.topgroup + "data/diffractionslices"][name]["data"]
            )
            if len(shape) == 2:
                dataobject = DiffractionSlice(
                    data=data, Q_Nx=Q_Nx, Q_Ny=Q_Ny, name=name
                )
            else:
                dim3 = info["dim3"]
                dataobject = DiffractionSlice(
                    data=data, slicelabels=dim3, Q_Nx=Q_Nx, Q_Ny=Q_Ny, name=name
                )
            ...
            if len(shape) == 2:
                ...
                dim3 = info["dim3"]
                dataobject = RealSlice(
                    data=data, slicelabels=dim3, R_Nx=R_Nx, R_Ny=R_Ny, name=name
                )
            ...
            for coord in coords:
                try:
                    dtype = type(
                        self.file[self.topgroup + "data/pointlists"][name][coord][
                            "data"
                        ][0]
                    )
                except ValueError:
                ...
                data_dict[coord] = np.array(
                    self.file[self.topgroup + "data/pointlists"][name][coord]["data"]
                )
            ...
            raise Exception(
                "Unknown object type {}. Returning None.".format(objecttype)
            )
    ...
    def get_dataobject_v0_6(self, index, **kwargs):
        ...
        if objecttype == "DataCube":
            ...
            for coord in coords:
                dtype = type(
                    self.file[self.topgroup + "data/pointlistarrays"][name]["0_0"][
                        coord
                    ]["data"][0]
                )
                coordinates.append((coord, dtype))
            ...
            for i in range(shape[0]):
                for j in range(shape[1]):
                    ...
                    for coord in coords:
                        data_dict[coord] = np.array(
                            self.file[self.topgroup + "data/pointlistarrays"][name][
                                "{}_{}".format(i, j)
                            ][coord]["data"]
                        )
    ...
    def get_dataobject_v0_5(self, index, **kwargs):
        ...
        if objecttype == "DataCube":
            ...
            if mmap:
                ...
                # TODO: preallocate array and copy into it for better RAM usage
                data = np.array(
                    self.file[self.topgroup + "data/datacubes"][name]["datacube"]
                )
            dataobject = DataCube(data=data, name=name)
            ...
            Q_Ny = shape[1]
            data = np.array(
                self.file[self.topgroup + "data/diffractionslices"][name][slices[0]]
            )
            if len(shape) == 2:
                dataobject = DiffractionSlice(
                    data=data,
            ...
            R_Ny = shape[1]
            data = np.array(
                self.file[self.topgroup + "data/realslices"][name][slices[0]]
            )
            if len(shape) == 2:
                dataobject = RealSlice(
                    data=data,
    ...
    def get_dataobject_v0_4(self, index, **kwargs):
        ...
        if objecttype == "DataCube":
            ...
            if mmap:
                ...
                # TODO: preallocate array and copy into it for better RAM usage
                data = np.array(
                    self.file["4DSTEM_experiment/data/datacubes"][name]["datacube"]
                )
            dataobject = DataCube(data=data, name=name)
            ...
            if depth == 1:
                data = np.array(
                    self.file["4DSTEM_experiment/data/diffractionslices"][name][
                        slices[0]
                    ]
                )
            else:
            ...
            dataobject = DiffractionSlice(
                data=data, slicelabels=slices, Q_Nx=Q_Nx, Q_Ny=Q_Ny, name=name
            )
            ...
            if depth == 1:
                data = np.array(
                    self.file["4DSTEM_experiment/data/realslices"][name][slices[0]]
                )
            else:
                data = np.empty((shape[0], shape[1], depth))
                for i in range(depth):
                    data[:, :, i] = np.array(
                        self.file["4DSTEM_experiment/data/realslices"][name][slices[i]]
                    )
            dataobject = RealSlice(
                data=data, slicelabels=slices, R_Nx=R_Nx, R_Ny=R_Ny, name=name
            )
            ...
            for coord in coords:
                try:
                    dtype = type(
                        self.file["4DSTEM_experiment/data/pointlists"][name][coord][
                            "data"
                        ][0]
                    )
                except ValueError:
                ...
                coordinates.append((coord, dtype))
                data_dict[coord] = np.array(
                    self.file["4DSTEM_experiment/data/pointlists"][name][coord]["data"]
                )
            dataobject = PointList(coordinates=coordinates, name=name)
            for i in range(length):
                new_point = tuple([data_dict[coord][i] for coord in coords])
            ...
            for coord in coords:
                dtype = type(
                    self.file["4DSTEM_experiment/data/pointlistarrays"][name]["0_0"][
                        coord
                    ]["data"][0]
                )
                coordinates.append((coord, dtype))
            ...
            for i in range(shape[0]):
                for j in range(shape[1]):
                    ...
                    for coord in coords:
                        data_dict[coord] = np.array(
                            self.file["4DSTEM_experiment/data/pointlistarrays"][name][
                                "{}_{}".format(i, j)
                            ][coord]["data"]
                        )
                    for k in range(len(data_dict[coords[0]])):
    ...
    def get_dataobject_v0_2(self, index):
        ...
        if objecttype == "RawDataCube":
            ...
            R_Nx, R_Ny, Q_Nx, Q_Ny = shape
            data = np.array(
                self.file["4DSTEM_experiment/processing/datacubes"][name]["datacube"]
            )
            dataobject = DataCube(data=data, name=name)
            ...
            if depth == 1:
                data = np.array(
                    self.file["4DSTEM_experiment/processing/diffractionslices"][name][
                        slices[0]
                    ]
                )
            else:
            ...
            if depth == 1:
                data = np.array(
                    self.file["4DSTEM_experiment/processing/realslices"][name][
                        slices[0]
                    ]
                )
            else:
            ...
            for coord in coords:
                dtype = type(
                    self.file["4DSTEM_experiment/processing/pointlists"][name][coord][
                        "data"
                    ][0]
                )
                coordinates.append((coord, dtype))
    ...
    def get_dataobjects(self, indices):
        if indices == "all":
            ...
            assert all(
                [isinstance(index, (int, np.integer)) for index in indices]
            ), "Arguments to get_dataobjects must be integers; to retrieve a dataobject by name, use get_dataobject"
        objects = []
...
def is_py4DSTEM_file(h5_file):
    ...
    if isinstance(h5_file, h5py._hl.files.File):
        if (
            ("version_major" in h5_file.attrs)
            and ("version_minor" in h5_file.attrs)
            and (
                ("4DSTEM_experiment" in h5_file.keys())
                or ("4D-STEM_data" in h5_file.keys())
            )
        ):
            return True
        else:
            ...
            for key in h5_file.keys():
                if (
                    ("version_major" in h5_file[key].attrs)
                    and ("version_minor" in h5_file[key].attrs)
                    and ("emd_group_type" in h5_file[key].attrs)
                ):
...
def get_py4DSTEM_version(h5_file):
    ...
    if isinstance(h5_file, h5py._hl.files.File):
        ...
        try:
            ...
            print(
                "Error: file cannot be opened with h5py, and may not be in HDF5 format."
            )
            return (0, 0)
...
def get_py4DSTEM_topgroup(h5_file):
    ...
    if isinstance(h5_file, h5py._hl.files.File):
        ...
        if len(topgroups) < 1:
            raise Exception(
                "No toplevel groups with appropriate tags (emd_group_type=2, version_major, version_minor) found."
            )
        return topgroups[0]
    else:
        f = h5py.File(h5_file, "r")
