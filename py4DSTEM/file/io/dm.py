...
class fileDM:
        ...
        try:
            ...
            if self._on_memory:
                ...
                with open(filename, "rb") as _fid:
                    if os.name == "nt":
                        self.fid = mmap.mmap(_fid.fileno(), 0, access=mmap.ACCESS_READ)
                    else:
                        self.fid = mmap.mmap(
                            _fid.fileno(), 0, prot=mmap.PROT_READ
    ...
    def fromfile(self, *args, **kwargs):
        ...
        if self._on_memory:
            if "dtype" not in kwargs:
                raise ValueError(
                    "In on_memory mode, reads require always a "
    ...
    def _validDM(self):
        ...
        self.fileSize = self.fromfile(self.fid, dtype=self.specialType, count=1)[
            0
        ]  # file size: real size - 24 bytes
        self.endianType = self.fromfile(self.fid, dtype=np.dtype(">u4"), count=1)[
            0
        ]  # endian type: 1 == little endian (Intel), 2 == big endian (old powerPC Mac)
    ...
    def _readTagGroup(self):
        ...
        if self.curGroupLevel > self.maxDepth:
            raise IOError(
                "Maximum tag group depth of {} reached. This file is most likely corrupt.".format(
                    self.maxDepth
                )
            )

        self.curGroupAtLevelX[self.curGroupLevel] = (
            self.curGroupAtLevelX[self.curGroupLevel] + 1
        )
        self.curTagAtLevelX[self.curGroupLevel] = 0
        self.fromfile(self.fid, dtype="<i1", count=2)  # is open and is sorted?
        nTags = self.fromfile(self.fid, dtype=self.specialType, count=1)[
            0
        ]  # needs to be read as Big Endian (.byteswap() could also work)
    ...
    def _readTagEntry(self):
        ...
        if self.v:
            print(
                "_readTagEntry: dataType = {}, lenTagLabel = {}".format(
                    dataType, lenTagLabel
                )
            )

        if lenTagLabel > 0:
            tagLabelBinary = self.fromfile(
                self.fid, dtype="<u1", count=lenTagLabel
            )  # read as binary
    ...
    def _readTagType(self):
        ...
        assert (
            delim == 37
        ).all()  # delim has to be [37,37,37,37] which is %%%% in ASCII.
        if self.v:
            print("_readTagType: should be %%%% = {}".format(self._bin2str(delim)))

        nInTag = self.fromfile(self.fid, dtype=self.specialType, count=1)[
            0
        ]  # nInTag: unnecessary redundant info

        # Determine the type of the data in the tag
        # specifies data type: int8, uint16, float32, etc.
        encodedType = self.fromfile(self.fid, dtype=self.specialType, count=1)[
            0
        ]  # big endian
        ...
        if etSize > 0:
            ...
            strTempBin = self.fromfile(
                self.fid, dtype="<u1", count=stringSize
    ...
    def _readStructTypes(self):
        ...
        """
        structNameLength = self.fromfile(self.fid, count=1, dtype=self.specialType)[
            0
        ]  # this is not needed
        ...
        for ii in range(0, nFields):
            aa = self.fromfile(
                self.fid, dtype=self.specialType, count=2
    ...
    def _readNativeData(self, encodedType):
        ...
        if encodedType == 2:
            ...
            val = self.fromfile(self.fid, count=1, dtype="<u1")[
                0
            ]  # return character or number?
            if self.v:
                print(
                    "_readNativeData untested type, val: {}, {}".format(
                        encodedType, val
                    )
                )
        elif encodedType == 9:  # matlab *char
            val = self.fromfile(self.fid, count=1, dtype="<i1")[
                0
            ]  # return character or number?
            if self.v:
                print(
            ...
            val = self.fromfile(self.fid, count=1, dtype="<u8")[
                0
            ]  # unknown type, but this works
        else:
        ...
        if self.v:
            print(
                "_readNativeData: encodedType == {} and val = {}".format(
                    encodedType, val
                )
            )
    ...
    def _readArrayData(self, arrayTypes):
        ...
        if self.v:
            print(
                "_readArrayData: arraySize, arrayTypes = {}, {}".format(
        ...
        if self.v:
            print(
                "_readArrayData: arraySize, itemSize = {}, {}".format(
                    arraySize, itemSize
                )
            )
        ...
        if self.curTagName == "Data":
            ...
            self.seek(
                self.fid, bufSize.astype("<u8"), 1
            )  # advance the pointer by bufsize from current position
            arrOut = "Data unread. Encoded type = {}".format(encodedType)
        elif (
            bufSize < 1e3
        ):  # set an upper limit on the size of array that will be read in as a string
            # treat as a string
            for encodedType in arrayTypes:
                stringData = self.fromfile(
                    self.fid, count=arraySize, dtype=self._encodedTypeDtype(encodedType)
                )
            ...
            if (fullTagName.find("Dimension") > -1) & (
                fullTagName.find("Units") > -1
            ):  # & (self.numObjects > 0)):
    ...
    def _storeTag(self, curTagName, curTagValue):
        ...
        if self.v:
            print(
                "_storeTag: curTagName, curTagValue = {}, {}".format(
                    curTagName, curTagValue
                )
            )
        totalTag = (
            self.curGroupNameAtLevelX + "." + "{}".format(curTagName)
        )  # + '= {}'.format(curTagValue)
    ...
    def _catchUsefulTags(self, totalTag, curTagName, curTagValue):
        ...
        if curTagName.find("Data.arraySize") > -1:
            ...
            self.ySize[
                -1
            ] = curTagValue  # OR self.ysize[self.numObjects] = self.curTagValue
    ...
    def _checkIndex(self, i):
        ...
        if i < 0 or i > self.numObjects:
            raise IndexError(
                "Index out of range, trying to access element {} of {} valid elements".format(
    ...
    def _DM2NPDataType(self, dd):
        ...
        #    return np.uint8 #bit1 ??
            ...
            print("4321")
            raise IOError(
                "Unsupported binary data type during conversion to numpy dtype. DM dataType == {}".format(
                    dd
                )
            )
    ...
    def getDataset(self, index):
        ...
        if self.xSize[ii] > 0:
            pixelCount = (
                int(self.xSize[ii]) * self.ySize[ii] * self.zSize[ii] * self.zSize2[ii]
            )
            ...
            if self.zSize[ii] == 1:  # 2D data
                ...
                ).reshape((self.ySize[ii], self.xSize[ii]))
                ...
                outputDict["data"] = self.fromfile(
                    ...
                    count=pixelCount,
                    dtype=self._DM2NPDataType(self.dataType[ii]),
                ).reshape((self.zSize[ii], self.ySize[ii], self.xSize[ii]))
                outputDict["pixelUnit"] = self.scaleUnit[jj : jj + self.dataShape[ii]][
                    ::-1
                ]  # need to reverse the order to match the C-ordering of the data
                outputDict["pixelSize"] = self.scale[jj : jj + self.dataShape[ii]][::-1]
    ...
    def _readRGB(self, xSizeRGB, ySizeRGB):
        ...
        return self.fromfile(
            self.fid, count=xSizeRGB * ySizeRGB * 4, dtype="<u1"
        ).reshape(xSizeRGB, ySizeRGB, 4)
