...
class PointList(DataObject):
    ...
    def __init__(self, coordinates, data=None, dtype=float, **kwargs):
        ...
        if type(coordinates) in (type, np.dtype):
            ...
            raise TypeError(
                "coordinates must be a list of strings, or a list of 2-tuples of structure (name, dtype)."
            )
    ...
    def add_unstructured_dataarray(self, data):
        ...
        """
        assert len(self.coordinates) == (
            data.shape[1]
        ), "Error: number of data columns must match the number of PointList coords."
        length = data.shape[1]
    ...
    def add_tuple_of_nparrays(self, data):
        ...
        assert len(self.coordinates) == len(
            data
        ), "Error: if data is a tuple, it must contain (# coords) numpy arrays."
        length = len(data[0])
        assert all(
            [length == len(ar) for ar in data]
        ), "Error: if data is a tuple, it must contain lists of equal length"
    ...
    def get_subpointlist(self, coords_vals):
        ...
        for tup in coords_vals:
            ...
            if len(tup) == 2:
                ...
                deletemask = (
                    deletemask
                    | (self.data[name] < minval)
                    | (self.data[name] >= maxval)
                )
...
class PointListArray(DataObject):
    ...
    def __init__(self, coordinates, shape, dtype=float, **kwargs):
        ...
        kwargs[
            "searchable"
        ] = False  # Ensure that the subpointlists don't all appear in searches
        self.pointlists = [
            [
                PointList(
                    coordinates=self.coordinates, dtype=self.default_dtype, **kwargs
                )
    ...
    def copy(self, **kwargs):
        ...
        """
        new_pointlistarray = PointListArray(
            coordinates=self.coordinates,
            shape=self.shape,
    ...
    def add_coordinates(self, new_coords, **kwargs):
        ...
        new_pointlistarray = PointListArray(
            coordinates=coords, shape=self.shape, dtype=self.default_dtype, **kwargs
        )
