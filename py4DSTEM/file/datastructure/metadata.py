...
class Metadata(object):
    ...
    def setup_search_dicts(self):
        ...
        self._search_dicts.original_to_comments_search_dict = {"comments": [""]}
    ...
    def setup_metadata_py4DSTEM(self, filepath, metadata_ind):
        ...
        if py4DSTEM_version == (0, 1):
            ...
            self.populate_original_metadata_from_h5_group(
                filepath[
                    topgroup
                    + "metadata/metadata_{}/original/shortlist".format(metadata_ind)
                ],
                self.original_metadata.shortlist,
            )
            self.populate_original_metadata_from_h5_group(
                filepath[
                    topgroup + "metadata/metadata_{}/original/all".format(metadata_ind)
                ],
                self.original_metadata.all,
            )

            # Copy metadata from .h5 groups to corresponding dictionaries
            self.populate_metadata_from_h5_group(
                filepath[
                    topgroup + "metadata/metadata_{}/microscope".format(metadata_ind)
                ],
                self.microscope,
            )
            self.populate_metadata_from_h5_group(
                filepath[topgroup + "metadata/metadata_{}/sample".format(metadata_ind)],
                self.sample,
            )
            self.populate_metadata_from_h5_group(
                filepath[topgroup + "metadata/metadata_{}/user".format(metadata_ind)],
                self.user,
            )
            self.populate_metadata_from_h5_group(
                filepath[
                    topgroup + "metadata/metadata_{}/calibration".format(metadata_ind)
                ],
                self.calibration,
            )
            self.populate_metadata_from_h5_group(
                filepath[
                    topgroup + "metadata/metadata_{}/comments".format(metadata_ind)
                ],
                self.comments,
            )
    ...
    def setup_metadata_py4DSTEM_file_v0_2_v0_3(self, filepath):
        ...
        self.populate_original_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/original/shortlist"],
            self.original_metadata.shortlist,
        )
        self.populate_original_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/original/all"],
            self.original_metadata.all,
        )

        # Copy metadata from .h5 groups to corresponding dictionaries
        self.populate_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/microscope"], self.data.microscope
        )
        self.populate_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/sample"], self.data.sample
        )
        self.populate_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/user"], self.data.user
        )
        self.populate_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/calibration"], self.data.calibration
        )
        self.populate_metadata_from_h5_group(
            filepath["4DSTEM_experiment/metadata/comments"], self.data.comments
        )
    ...
    def setup_metadata_py4DSTEM_file_v0_1(self, filepath):
        ...
        self.original_metadata.all = MetadataCollection("all")
        self.populate_original_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/original/shortlist"],
            self.original_metadata.shortlist,
        )
        self.populate_original_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/original/all"], self.original_metadata.all
        )

        # Copy metadata from .h5 groups to corresponding dictionaries
        self.populate_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/microscope"], self.data.microscope
        )
        self.populate_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/sample"], self.data.sample
        )
        self.populate_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/user"], self.data.user
        )
        self.populate_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/calibration"], self.data.calibration
        )
        self.populate_metadata_from_h5_group(
            filepath["4D-STEM_data/metadata/comments"], self.data.comments
        )

    def populate_original_metadata_from_h5_group(
        self, h5_metadata_group, target_metadata_group
    ):
        if len(h5_metadata_group.attrs) > 0:
            target_metadata_group.metadata_items = dict()
            self.populate_metadata_from_h5_group(
                h5_metadata_group, target_metadata_group.metadata_items
            )
        for subgroup_key in h5_metadata_group.keys():
            vars(target_metadata_group)[subgroup_key] = MetadataCollection(subgroup_key)
            self.populate_original_metadata_from_h5_group(
                h5_metadata_group[subgroup_key],
                vars(target_metadata_group)[subgroup_key],
            )
...
def get_py4DSTEM_topgroup(h5_file):
    ...
    ):  # or ('4D-STEM_data' in h5_file.keys()) or ('4DSTEM_simulation' in h5_file.keys())):
